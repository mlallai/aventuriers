...
class Adventure extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (adventure.adventure) {
      ...
      const ecoLabel = adventure.adventure.ecoLabel;
      this.setState({
        loadedAdventure: loadedAdventure,
        comments: comments,
        pictures: pictures,
        ecoLabel: ecoLabel,
        loading: false
      ...
      if (loadedProfile) {
        this.setState({
          ...
          loadedProfileSports: loadedProfile.sports
        });
      }
    }
  ...
  onSubmit(e) {
    ...
    this.props.addMessage(
      this.props.adventure.adventure.user._id,
      messData,
      this.props.history
    );
  }
  ...
  render() {
    ...
    if (this.state.isLiked === false) {
      wish = (
        <Button
          onClick={() => this.onLikeClick(loadedAdventure._id)}
          round
          color="warning"
        >
          <StarRate style={{ color: "black" }} />{" "}
      ...
      wish = (
        <Button
          onClick={() => this.onDislikeClick(loadedAdventure._id)}
          round
          color="danger"
        >
    ...
    if (
      ...
      if (pics[0]) {
        ...
        images = (
          <div>
            <img src={loadedAdventure.defaultPictures[0]} alt="" />
          </div>
        );
      }
    }
    ...
    let loadedEcoLabel;
    if (ecoLabel === undefined || null || loading) {
      loadedEcoLabel = <Spinner />;
    } else {
      if (ecoLabel.length > 0) {
        loadedEcoLabel = ecoLabel.map(item => (
          <p
            style={{
              marginBottom: "3px",
              fontStyle: "italic",
    ...
    if (comments === undefined) {
      loadedComments = <Spinner />;
    } else {
      loadedComments = comments.map(comment => (
        <CommentItem
          key={comment._id}
          comment={comment}
          adventureId={comment.id}
        />
      ));
    }
        ...
        >
          <GridContainer>
            ...
            >
                ...
                <GridItem xs={12} sm={12} md={12}>
                  {loadedUser.avatar ? (
                    ...
                    <GridContainer style={{ justifyContent: "center" }}>
                      <Avatar style={{ backgroundColor: "#2a8afa" }}>
                        {loadedUser.defaultAvatar}
                      </Avatar>
                    </GridContainer>
                  )}
                </GridItem>
              ...
              <br />
              <GridContainer style={{ justifyContent: "center" }}>
                {loadedProfile.age ? <p>{loadedProfile.age} ans,&nbsp;</p> : ""}
                {loadedProfile.location ? (
                  <p>{loadedProfile.location},&nbsp;</p>
                ) : (
                  ""
                )}
                {loadedProfile.country ? <p>{loadedProfile.country}</p> : ""}
              </GridContainer>
              <GridContainer style={{ justifyContent: "center" }}>
                {loadedProfile.desc ? (
            ...
            <GridItem md={12} sm={12} style={{ textAlign: "center" }}>
              {isAuthenticated ? (
                <Button
                  ...
                  color="default"
                >
                  <MailOutline style={{ color: "#ffffff" }} />{" "}
    ...
    if (loading) {
      ...
      advContent = (
        ...
        >
          ...
          <br />
          <GridContainer
            style={{ width: "100%", marginleft: "0px", marginRight: "Opx" }}
          >
            <GridItem
              xs={12}
              sm={6}
            ...
            >
              ...
              <GridContainer>
                ...
                <GridItem xs={4}>
                  ...
                  >
                    {loadedAdventure.level}
                  </p>
                  {loadedAdventure.requiredPeople ? (
                    <p
                      style={{
                        lineHeight: "1.5em",
                        color: "black",
                    ...
                    >
                      {loadedAdventure.requiredPeople} équipiers
                    </p>
                  ) : (
                    ""
                  )}
                  ...
                  {loadedAdventure.from ? (
                    <div>
                      ...
                      {loadedAdventure.duration ? (
                        ...
                        >
                          {loadedAdventure.duration} jours
                        </p>
                      ) : (
                        ""
                      )}
          ...
          <GridContainer>
            ...
            <GridItem md={7} sm={7}>
              <div>{loadedEcoLabel}</div>
              <div>
                <h3 style={{ marginTop: "0px" }} className={classes.title}>
                  A propos
                </h3>
                <br />
                <br />
                <p>
                  {loadedAdventure.summary
                    ? loadedAdventure.summary
                    : "Pas de résumé"}
                </p>
              </div>
          ...
          <GridContainer>
            <GridItem xs={12}>
              <div>
                ...
                <br />
                {isAuthenticated ? (
                  <CommentForm adventureId={loadedAdventure._id} />
                ) : (
                  <h5>Vous devez être connecté pour poster un commentaire</h5>
                )}
...
export default connect(
  mapStateToProps,
  { getAdventure, addToWishList, removeFromWishList, addMessage }
)(withStyles(productStyle)(Adventure));
