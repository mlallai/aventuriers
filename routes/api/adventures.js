...
router.get("/", (req, res) => {
  Adventure.find()
    ...
    .catch(err =>
      res.status(404).json({ noadventurefound: "No Adventure found" })
    );
});
...
router.get("/adventure/:id", (req, res) => {
  Adventure.findById(req.params.id)
    ...
    .populate("profile", ["age", "location", "desc", "country", "sports"])
    // .populate('profile', ['age', 'location', 'country', 'sports'])
    .then(adventure => res.json(adventure))
    .catch(err =>
      res
        .status(404)
        .json({ noadventurefound: "No adventure found with that ID" })
    );
});
...
router.get(
  ...
  (req, res) => {
    Adventure.find({ user: req.user.id })
      ...
      .then(adventure => res.json(adventure))
      .catch(err =>
        res.status(404).json({ noadventurefound: "No adventures found" })
      );
    // });
  }
);
...
router.post(
  ...
  (req, res) => {
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      adventureFields.profile = profile;
    });

    if (req.body.title) adventureFields.title = req.body.title;
    if (req.body.mainActivity)
      adventureFields.mainActivity = req.body.mainActivity;
    ...
    if (req.body.mainActivity === "Alpinisme") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720348/Alpinism.jpg"
      );
    }
    if (req.body.mainActivity === "Canyoning") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720348/Canyoning.jpg"
      );
    }
    if (req.body.mainActivity === "Cyclisme") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720349/Cycling.jpg"
      );
    }
    if (req.body.mainActivity === "Equitation") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720349/Horse_riding.jpg"
      );
    }
    if (req.body.mainActivity === "Escalade") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720349/Climbing.jpg"
      );
    }
    if (req.body.mainActivity === "Canoë-kayak") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720349/Kayaking.jpg"
      );
    }
    if (req.body.mainActivity === "Kitesurf") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720349/Kitesurf.jpg"
      );
    }
    if (req.body.mainActivity === "Paddle") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1546874587/Paddle.jpg"
      );
    }
    if (req.body.mainActivity === "Parachute") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720349/Parachute.jpg"
      );
    }
    if (req.body.mainActivity === "Parapente") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720350/Paragliding.jpg"
      );
    }
    if (req.body.mainActivity === "Planche à voile") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720348/Windsurfing.jpg"
      );
    }
    if (req.body.mainActivity === "Plongée") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720348/Diving.jpg"
      );
    }
    if (req.body.mainActivity === "Randonnée") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720349/Hiking2.jpg"
      );
    }
    if (req.body.mainActivity === "Ski") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720350/Skiing.jpg"
      );
    }
    if (req.body.mainActivity === "Ski de fond") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720350/Ski_touring.jpg"
      );
    }
    if (req.body.mainActivity === "Ski de randonnée") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547722293/ski_touring_3.jpg"
      );
    }
    if (req.body.mainActivity === "Snowboard") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720350/Snowboarding.jpg"
      );
    }
    if (req.body.mainActivity === "Surf") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720348/Surf.jpg"
      );
    }
    if (req.body.mainActivity === "Trail") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720348/Trail.jpg"
      );
    }
    if (req.body.mainActivity === "Trek") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547722293/hiking_3.jpg"
      );
    }
    if (req.body.mainActivity === "Voile") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720350/Sailing.jpg"
      );
    }
    if (req.body.mainActivity === "Voyage à cheval") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547722293/horse_trip1.jpg"
      );
    }
    ...
    if (req.body.mainActivity === "Voyage à vélo") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547722293/bike_trip1.jpg"
      );
    }
    if (req.body.mainActivity === "VTT") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720349/Mountain_bike.jpg"
      );
    }
    if (req.body.mainActivity === "Wakeboard") {
      adventureFields.defaultPictures.push(
        "https://res.cloudinary.com/adventurer/image/upload/v1547720348/Wakeboard.jpg"
      );
    }

    Adventure.findById(req.params.id).then(adventure => {
      if (adventure) {
        // Update
  ...
  (req, res) => {
    ...
    User.findOne({ user: req.user.id }).then(user => {
      Adventure.findById(req.params.id)
        .then(adventure => {
          ...
          if (adventure.user.toString() !== req.user.id) {
            ...
            return res.status(401).json({
              notauthorized: "User not authorized to delete Adventure"
            });
          }
        ...
        .catch(err =>
          res.status(404).json({ adventurenotfound: "No Adventure found" })
        );
    });
  }
);
    ...
    User.findOne({ user: req.user.id }).then(user => {
      Adventure.findById(req.params.id)
        .then(adventure => {
          ...
          if (
            adventure.likes.filter(like => like.user.toString() === req.user.id)
              .length > 0
          ) {
            return res.status(400).json({
              alreadyliked: "L'utilisateur a déjà enregistré cette aventure"
            });
          }
      ...
      Adventure.findById(req.params.id)
        .then(adventure => {
          ...
          if (
            ...
            return res
              .status(400)
              .json({ notliked: "you have not yet liked this adventure" });
          }
